...
bump cannot drop a copyright line unnoticed. Before a tag, `oss provenance
sucher` searches GitHub for the most distinctive identifiers added since the
last release and fails on a copyleft match that `THIRD_PARTY.md` doesn't
already explain. It runs from the fleet tool rather than from this repository,
and a monthly sweep repeats it across every project.
